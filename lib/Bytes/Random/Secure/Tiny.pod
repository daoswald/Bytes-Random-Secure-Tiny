...
=head1 SYNOPSIS
    ...
    my $hex    = $rng->bytes_hex(10);          # 10 randombytes as hex digits.
...
the reference. This means that C<$aref> and C<$aref_shuffled> are the same
reference, and that C<$aref> is shuffled in place.

L<List::Util> comes with a C<shuffle> function, but that shuffle is based on
Perl's C<rand>, and thus, unsatisfactory for uses where a cryptographic
quality C<rand> is required.  L<Bytes::Random::Secure::Tiny>'s C<shuffle>
uses a CSPRNG for higher quality randomness.

Furthermore, the Fisher-Yates shuffle as implemented by List::Util suffers from
a problem documented by Abigail in L<Algorithm::Numerical::Shuffle>: Perl's
C<rand> is typically seeded using a 32-bit seed. Since a shuffle using a PRNG
depends on three things (the seed, the algorithm, and the input), the outcome
of a given shuffle is purely based on the seed.
of the shuffle is purely based on the seed. This means that on a system with
32-bit random numbers, there are at most 2^32 possible shuffles. But for a list
of C<n> elements, there are C<n!> possible permutations. Consequently, a list
as small as 13 elements cannot shuffle in certain permutations.

This module's C<shuffle> benefits from large seeds and a huge internal state.
ISAAC can be seeded with up to 8192 bits, yielding 2^8192 possible initial
states, with 2^8288 possible internal states.
=head1 CONFIGURATION
